...
\n\
\n\
__kernel void activate_array_kernel(__global float *x, int x_offset, int n, int a)\n\
{\n\
    int i = get_global_id(2) * get_global_size(0) * get_global_size(1) +\n\
		get_global_id(1) * get_global_size(0) + get_global_id(0);\n\
    if(i < n) x[x_offset + i] = activate_kernel(x[x_offset + i], a);\n\
}\n\
\n\
...
{\n\
    ...
    if(offset < size) output[output_offset + global_z*size + offset] *= biases[biases_offset + filter];\n\
}\n\
...
__kernel void add_bias_kernel(__global float *output, int output_offset, __global float *biases, int biases_output, int batch, int n, int size)\n\
{\n\
	...
	index /= n;\n\
	int k = index;\n\
    output[output_offset + (k*n + j)*size + i] += biases[biases_output + j];\n\
}\n\
...
{\n\
    ...
    x[x_offset + index] = (x[x_offset + index] - mean[mean_offset + f])/(sqrt(variance[variance_offset + f] + .00001f));\n\
}\n\
...
__kernel void copy_kernel(int N, __global float *X, int X_offset, int OFFX, int INCX,\n\
                                 __global float *Y, int Y_offset, int OFFY, int INCY)\n\
{\n\
    int i = get_global_id(2) * get_global_size(0) * get_global_size(1) +\n\
		get_global_id(1) * get_global_size(0) + get_global_id(0);\n\
    if(i < N) Y[i*INCY + OFFY + Y_offset] = X[i*INCX + OFFX + X_offset];\n\
}\n\
...
__kernel void upsample_kernel(int N, __global float *x, int x_offset, \
                              int w, int h, int c, int batch, int stride, int forward, float scale, \
                              __global float *out, int out_offset)\n\
{\n\
    int i = get_global_id(2) * get_global_size(0) * get_global_size(1) +\n\
                                    get_global_id(1) * get_global_size(0) + get_global_id(0);\n\
    if(i >= N) return;\n\
    ...
    if(forward) out[out_offset + out_index] += scale * x[x_offset + in_index];\n\
    else atomic_float_add(x+in_index, scale * out[out_offset + out_index]);\n\
}\n\
"; 
...
*/\n\
\n\
__kernel void im2col_gpu_kernel(const int n, __global const float* data_im, const int im_offset,\n\
        const int height, const int width, const int ksize,\n\
        const int pad,\n\
        const int stride,\n\
        const int height_col, const int width_col,\n\
        __global float *data_col, const int col_offset) {\n\
	int index = get_global_id(1) * get_global_size(0) + get_global_id(0);\n\
	for(; index < n; index += get_global_size(1) * get_global_size(0)){\n\
        int w_out = index % width_col;\n\
        int h_index = index / width_col;\n\
        ...
        int w_in = w_out * stride - pad;\n\
\n\
        int data_col_offset = col_offset + (channel_out * height_col + h_out) * width_col + w_out;\n\
        int data_im_offset = im_offset + (channel_in * height + h_in) * width + w_in;\n\
        for (int i = 0; i < ksize; ++i) {\n\
            for (int j = 0; j < ksize; ++j) {\n\
                int h = h_in + i;\n\
                int w = w_in + j;\n\
...
__kernel void forward_maxpool_layer_kernel(int n, int in_h, int in_w, int in_c, int stride, int size, int pad, \
                                          __global float *input, int input_offset, \
                                          __global float *output, int output_offset, \
                                          __global int   *indexes, int indexes_offset)\n\
{\n\
    ...
    for(l = 0; l < size; ++l){\n\
        for(m = 0; m < size; ++m){\n\
            ...
            int valid = (cur_h >= 0 && cur_h < in_h &&\n\
                    cur_w >= 0 && cur_w < in_w);\n\
            float val = (valid != 0) ? input[input_offset + index] : -INFINITY;\n\
            max_index = (val > max_value) ? index : max_index;\n\
            max_value   = (val > max_value) ? val   : max_value;\n\
        }\n\
    }\n\
    output[output_offset + out_index] = max_value;\n\
    indexes[indexes_offset + out_index] = max_index;\n\
}\n\
\n\
