...
public:
    CLArray();
    CLArray(std::shared_ptr<cl_mem> buffer, size_t originSize, size_t offset, size_t subSize);
    ~CLArray();
    ...
    std::shared_ptr<cl_mem> m_buffer;
    size_t m_originSize;   // in elements
    size_t m_regionOffset; // in elements
    size_t m_regionSize;   // in elements

    const cl_mem& getBuffer() const { return *m_buffer; }
};
#endif
